...
public:
    ...
    Spectrum eval(const BSDFSamplingRecord &_bRec, EMeasure measure) const
    {
        ...
        std::vector<Float> m_alphasU(nb_layers);
        // Float m_alphasV[nb_layers];
        std::vector<Float> m_alphasV(nb_layers);
        // Spectrum m_reflectances[nb_layers];
        std::vector<Spectrum> m_reflectances(nb_layers);
        // Vector m_normals[nb_layers];
        std::vector<Vector> m_normals(nb_layers);
        // Spectrum m_kappas[nb_layers+1], m_etas[nb_layers+1];
        std::vector<Spectrum> m_kappas(nb_layers + 1);
        std::vector<Spectrum> m_etas(nb_layers + 1);
        // Spectrum m_sigmas[nb_layers], m_sigmaa[nb_layers];
        std::vector<Spectrum> m_sigmas(nb_layers);
        std::vector<Spectrum> m_sigmaa(nb_layers);
